-- Luna Voice / stage 5. Paste this entire file into Supabase SQL Editor and Run.
-- Safe to run again. Does not create fake users, erase history, or change Auth settings.
begin;
...
create table if not exists public.voice_records (
  ...
  kind text not null check (kind in ('attempt','session','setting','favorite','profile')),
  id text not null check (length(id) between 1 and 100),
  data jsonb,
  deleted boolean not null default false,
  updated_at timestamptz not null default now(),
  primary key (owner_id, kind, id),
  check (data is null or octet_length(data::text) <= 100000)
);
create table if not exists voice_private.receipts (
  owner_id uuid not null references auth.users(id) on delete cascade,
...
alter table voice_private.receipts enable row level security;
revoke all on public.voice_records from public, anon, authenticated;
grant select on public.voice_records to authenticated;
drop policy if exists voice_read_own on public.voice_records;
create policy voice_read_own on public.voice_records for select to authenticated
  using (owner_id = (select auth.uid()));

-- Only the authenticated RPC below may mutate records. No direct client writes.
create or replace function voice_private.put_record(u uuid, k text, i text, d jsonb, dead boolean default false)
returns void language plpgsql set search_path = '' as $$
begin
  insert into public.voice_records(owner_id,kind,id,data,deleted)
    values(u,k,i,case when dead then null else d end,dead)
  on conflict(owner_id,kind,id) do update set
    data = case when excluded.deleted then null else excluded.data end,
    deleted = excluded.deleted, updated_at = now()
  where not voice_records.deleted and
    (excluded.deleted or k not in ('attempt','session') or voice_records.data->>'status' = 'incomplete');
end;
$$;
revoke all on function voice_private.put_record(uuid,text,text,jsonb,boolean) from public, anon, authenticated;
  ...
  if k in ('attempt','session') then
    ...
    if k = 'attempt' then
      if jsonb_typeof(r->'practice') is distinct from 'object' or jsonb_typeof(r->'audio') is distinct from 'object'
        ...
        or coalesce(r->'practice'->>'exerciseId','') not in ('ladder','steady','pulse','descent','thirds','melody')
        or coalesce((r->'practice'->>'baseMidi')::numeric,0) not between 48 and 77
    ...
    for pair in select * from jsonb_each(mutation->'value') loop
      if pair.key = 'tempos' then
        ...
        for i,r in select * from jsonb_each(pair.value) loop
          if i not in ('ladder','steady','pulse','descent','thirds','melody') or jsonb_typeof(r) <> 'number' or r::text::numeric not between 30 and 150 then raise exception 'Invalid tempo'; end if;
          perform voice_private.put_record(u,'setting','tempo:'||i,r);
        end loop;
      else
        if not (
          (pair.key='baseMidi' and jsonb_typeof(pair.value)='number' and pair.value::text::numeric between 48 and 77) or
          ...
          (pair.key='lastExerciseId' and pair.value #>> '{}' in ('ladder','steady','pulse','descent','thirds','melody'))
    ...
    i := mutation->>'id';
    if i is null or i not in ('ladder','steady','pulse','descent','thirds','melody') or jsonb_typeof(mutation->'enabled') is distinct from 'boolean' then raise exception 'Invalid favorite'; end if;
...
-- No user ID parameter: a caller can delete only their own account.
create or replace function public.voice_delete_account()
returns void language plpgsql security definer set search_path = '' as $$
declare u uuid := auth.uid();
begin
  if u is null or coalesce((select max((entry->>'timestamp')::bigint) from jsonb_array_elements(coalesce(auth.jwt()->'amr','[]'::jsonb)) entry where entry->>'method'='otp'),0) < extract(epoch from now()) - 900 then
...
comment on table public.voice_records is 'Luna Voice v1: owner-scoped derived practice records; deletion tombstones prevent offline resurrection.';
commit;
...
-- Expanded exercise catalogue. Preserves history and account settings.
begin;
create or replace function public.voice_apply(operation_id uuid, mutation jsonb)
returns void language plpgsql security definer set search_path = '' as $$
declare
